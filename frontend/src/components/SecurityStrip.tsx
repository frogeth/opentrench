...
const tone = (n: number | undefined, warn: number, bad: number) => (n === undefined ? '' : n >= bad ? ' bad' : n >= warn ? ' warn' : ' ok');
 ...
 */
export function SecurityStrip({ s }: { s: TokenSecurity }) {
  ...
  const all: Item[] = [
    ...
    { icon: 'top', text: pct(s.top10Pct), tip: `Top 10 holders hold ${pct(s.top10Pct) ?? '?'}`, cls: tone(s.top10Pct, 25, 50) },
    { icon: 'dev', text: pct(s.devPct), tip: s.devSold ? 'Dev sold all' : `Dev holds ${pct(s.devPct) ?? '?'}`, cls: tone(s.devPct, 5, 15) },
    { icon: 'insider', text: pct(s.insidersPct), tip: `Insiders hold ${pct(s.insidersPct) ?? '?'}`, cls: tone(s.insidersPct, 10, 25) },
    { icon: 'sniper', text: pct(s.snipersPct), tip: `Snipers hold ${pct(s.snipersPct) ?? '?'}`, cls: tone(s.snipersPct, 10, 25) },
    { icon: 'bundle', text: pct(s.bundlersPct), tip: `Bundlers hold ${pct(s.bundlersPct) ?? '?'}`, cls: tone(s.bundlersPct, 10, 25) },
  ...
  const items = all.filter((i) => i.text !== undefined);
  const tax = s.buyTax !== undefined || s.sellTax !== undefined ? `${pct(s.buyTax) ?? '?'}/${pct(s.sellTax) ?? '?'}` : undefined;
  return (
    <div className="sec">
      {items.map((i) => (
