 ...
 * fetches go out on, so only a local process may claim it. A loopback peer can arrive as the mapped
 * v6 spelling (`::ffff:127.0.0.1`), which `isLocalHost` reads the way a URL writes IPv6: bracketed.
 */
export function isLoopbackCaller(address: string): boolean {
  const a = String(address ?? '').trim();
  if (!a) return false;
  return isLocalHost(a) || isLocalHost(`[${a}]`);
}

/** What a plugin is allowed to put on the wire. `ShellLink` decides what it may see coming back. */
function pluginHeaders(raw: unknown, viaShell: boolean): Record<string, string> {
...
export function createPluginsApi(
  ...
  r.post('/plugins/add-url', async (req, res) => {
    ...
    try {
      const source = await download(u.toString(), fetchImpl, (addUrlAgent ??= safeDispatcher()));
      res.json(reg.add(source));
    } catch (e) {
      fail(res, 400, why(e));
    }
  ...
  r.post('/plugins/:id/fetch', async (req, res) => {
    ...
    const body = typeof init.body === 'string' ? init.body.slice(0, FETCH_BODY_MAX) : undefined;
