...
- TrenchTogether rooms: share your calls with friends anywhere, not only on the same Wi-Fi. Settings → Together → Create a room, Copy invite, and one link joins them; their calls show `via <name>` in your feed and yours in theirs. Only calls travel, never chat messages, and each app prices tokens itself. Rotate makes a new invite; anyone with the old one is out.
...
- The relay is in the repo (`relay/`): one Node process, no database, keeps the last day of a room for late joiners. Host your own on Fly.io, in Docker, or behind Caddy on any host; an optional access code keeps it private to your circle. The app ships a default relay address prefilled in the create form. Docs: docs/relay.md.
...
- `opentrench://` links open the desktop app on Settings → Together with the invite filled in; the app registers the URL scheme on macOS, Windows and Linux.
